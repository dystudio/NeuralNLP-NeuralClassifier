...
def data_process(df, outfile, tokenize_strategy):
    ...
    with open(outfile, "w+", buffering=20, encoding='utf-8') as f:
        ...
        for indexs in df.index:
            ...
            dict1 = {}
            print('""""""""""test doc_label start"""""""""""""')
            print(str(df.loc[indexs].values[0]))
            print('""""""""""test doc_label end"""""""""""""')
            doc_label = str(df.loc[indexs].values[0])
            if len(doc_label) != 0:
                print ("this is not an empty string")
                if label_separator in doc_label:
                    print ("this is with | string")
                    dict1['doc_label'] = doc_label.split('|')
                else:
                    print ("this is without | string")
                    dict1['doc_label'] = [doc_label]
            else:
...
threads = []
t1 = threading.Thread(target=data_process,args=(train_data_df, input_path + 'rcv3_train.json', "jieba"))
threads.append(t1)
t2 = threading.Thread(target=data_process,args=(test_data_df, input_path + 'rcv3_dev.json', "jieba"))
threads.append(t2)
t3 = threading.Thread(target=data_process,args=(df2, input_path + 'rcv3_test.json', "jieba"))
threads.append(t3)
